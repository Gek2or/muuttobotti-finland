...
import { adminOperation, Booking, getAdminOperations, upsertWorker, Worker } from '../api';
...
export default function AdminOperationsScreen() {
  ...
  const load = useCallback(async (
    ...
    try {
      const result = await getAdminOperations(value.trim(), filters);
  ...
  const closeBooking = () => {
    setSelected(null); setQuote(''); setNote(''); setFinalPrice(''); setActualHours('');
    void load(token,{q,status,date},true);
  };
  ...
  const logout = async () => {
    setAuthorized(false); setToken(''); setSelected(null);
    try { await secureStorage.clearAdminToken(); } finally { router.replace('/'); }
  };

  if (!authorized) return (
    <ScrollView contentContainerStyle={s.wrap} keyboardShouldPersistTaps="handled">
      <View style={s.login}><Text style={s.kicker}>MUUTTOBOTTI · OPERATIONS V2.1</Text><Text style={s.loginTitle}>Connecting…</Text><Text style={s.copy}>{message || 'SecureStore → Operations API'}</Text></View>
    </ScrollView>
  );

  if (selected) return (
    <ScrollView contentContainerStyle={s.wrap} refreshControl={<RefreshControl refreshing={busy} onRefresh={()=>void load(token,{q,status,date})} tintColor={colors.ink}/>} keyboardShouldPersistTaps="handled">
      <TouchableOpacity onPress={closeBooking}><Text style={s.back}>← Varaukset</Text></TouchableOpacity>
      <View style={s.detailHero}>
        <View style={s.liveRow}><Text style={s.kicker}>{selected.id}</Text><View style={s.live}><Text style={s.liveText}>● LIVE</Text></View></View>
        <Text style={s.detailTitle}>{selected.customer_name}</Text>
        <Text style={s.detailSub}>{statusFi[selected.status]||selected.status} · {selected.preferred_date} {selected.preferred_time}</Text>
        <Text style={s.route}>{selected.pickup} → {selected.destination}</Text>
      </View>
      ...
      <Section title="VALMISTUMINEN"><View style={s.two}><TextInput keyboardType="decimal-pad" value={finalPrice} onChangeText={setFinalPrice} placeholder="Lopullinen €" style={[s.input,{flex:1}]}/><TextInput keyboardType="decimal-pad" value={actualHours} onChangeText={setActualHours} placeholder="Tunnit" style={[s.input,{flex:1}]}/></View><TouchableOpacity disabled={busy} style={s.complete} onPress={()=>void run('complete',{final_price:Number(finalPrice.replace(',','.')),actual_hours:Number(actualHours.replace(',','.'))})}><Text style={s.completeText}>Merkitse valmiiksi</Text></TouchableOpacity></Section>

      <Section title="ADMIN NOTE"><TextInput multiline value={note} onChangeText={setNote} placeholder="Sisäinen huomio" style={[s.input,s.textarea]}/><TouchableOpacity disabled={busy} style={s.secondary} onPress={()=>void run('note',{note})}><Text style={s.secondaryText}>Tallenna huomio</Text></TouchableOpacity></Section>

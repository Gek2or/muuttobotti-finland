...
export default function TrackingClient() {
  ...
  return <main className="tracking-page">
    <header className="tracking-header">
      ...
      <div className="tracking-languages">{(Object.keys(languages) as Locale[]).map(language => <button className={language === locale ? "active" : ""} onClick={() => setLocale(language)} key={language}>{languages[language]}</button>)}</div>
    ...
    <section className="tracking-shell">
      ...
      <AnimatePresence mode="wait">
        {!booking ? <motion.form className="tracking-lookup" key="lookup" onSubmit={submitLookup} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
          ...
          <div className="tracking-dashboard-head"><div><span>{t.hello}, {booking.customer_name}</span><h2>{booking.id}</h2></div><div className={`tracking-status status-${booking.status}`}>{booking.status.replaceAll("_", " ")}</div></div>
          ...
          {canEdit && <div className="tracking-actions"><button onClick={() => setEditing(true)}><Edit3/>{t.change}</button><button className="danger" onClick={() => window.confirm(t.confirmCancel) && void updateBooking("cancel")}><Trash2/>{t.cancel}</button></div>}
        </motion.div>}
      </AnimatePresence>
    </section>
